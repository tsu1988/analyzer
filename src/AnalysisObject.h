#ifndef Podd_AnalysisObject
#define Podd_AnalysisObject
...
class AnalysisObject : public TNamed {
...
public:
  ...
  AnalysisObject();  // only for ROOT I/O
...
protected:
  ...
  AnalysisObject*      FindModule( const char* name, const char* classname,
				   bool do_error = true );
  ...
  AnalysisObject( const char* name, const char* description );

private:
  // Prevent default construction, copying, assignment
  AnalysisObject( const AnalysisObject& );
  AnalysisObject& operator=( const AnalysisObject& );

  static TList* fgModules;  // List of all currently existing Analysis Modules

  ClassDef(AnalysisObject,1)   //ABC for a data analysis object
};

}  // end namespace Podd

// Backwards-compatibility
#define THaAnalysisObject Podd::AnalysisObject
